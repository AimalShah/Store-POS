...
export async function printReceiptJob(tx, settings, config, { printKot = false } = {}) {
  console.log('printReceiptJob: Starting');
  const printer = makePrinter(config.receipt);
  if (!printer) {
    console.error('printReceiptJob: makePrinter failed, returning fallback');
    return { printed: false, fallback: true, kotPrinted: false };
  }
  try {
    writeReceipt(printer, tx, settings);
    console.log('printReceiptJob: Executing printer...');
    await printer.execute();
    console.log('printReceiptJob: Success');
  } catch (err) {
